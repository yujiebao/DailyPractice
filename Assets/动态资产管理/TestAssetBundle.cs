...
public class TestAssetBundle : MonoBehaviour
{
    ...
    private void Start() {
        ...
        // AssetBundle myAb = AssetBundle.LoadFromFile(bundlePath+"/"+"prefabs");
        // GameObject sphere = myAb.LoadAsset<GameObject>("Sphere");
        // Instantiate(sphere);
        ...
        // string[] dependencies = manifest.GetAllDependencies("prefabs");    //当前包的所有依赖的包
        // foreach(string dependency in dependencies)
        // {
        //     AssetBundle.LoadFromFile(bundlePath+"/"+dependency);
        // }
        // AssetBundle myAb = AssetBundle.LoadFromFile(bundlePath+"/"+"prefabs");
        // GameObject sphere = myAb.LoadAsset<GameObject>("Cube");
        // Instantiate(sphere);
    ...
    //    ！！！-----有问题异步操作不适合写在for里面，for执行较快
        ...
        UnityWebRequest request = UnityWebRequestAssetBundle.GetAssetBundle("http://localhost:8080/AssetBundles/prefabs");
        yield return request.SendWebRequest();
        AssetBundle bundle = DownloadHandlerAssetBundle.GetContent(request); //bundle就是ab包可以获取ab包中的内容
        GameObject Sphere = bundle.LoadAsset<GameObject>("Cube");
        Instantiate(Sphere);
    }
        //异步加载
